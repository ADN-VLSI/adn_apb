...
module adn_apb_cdc_tb;
  ...
  task automatic apb_read(input logic [31:0] addr, input logic [31:0] data);
    ...
    wait_cycles = 0;
    while (!mst_rsp.pready && wait_cycles < TIMEOUT_CYCLES) begin
      @(posedge mst_clk);
      wait_cycles++;
    end
    ...
    if (mst_rsp.prdata !== data) begin
      $display("Error: response data mismatch. Expected %h, Got %h", data, mst_rsp.prdata);
      pass = 0;
    end

    if (!mst_rsp.pready) begin
  ...
  task automatic apb_write_multi_attempt();
    ...
    repeat (10) begin
      ...
      if (slv_req.psel && slv_req.penable) begin
        seen_count++;
        $display(
            "INFO: slave saw an ADDITIONAL transaction: addr=%h data=%h (not expected for APB)",
